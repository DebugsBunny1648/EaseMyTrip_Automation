...
//page-->fixture-->global varaiable:page,broswer
...
test("Test Register Page", async ({ page }) => {
    ...
    const totalGender = await page.locator("input[name='radiooptions']");
    console.log("Total Genders: " + await totalGender.count());

    for (let i = 0; i < await totalGender.count(); i++) {
        const rowValue = totalGender.nth(i);
